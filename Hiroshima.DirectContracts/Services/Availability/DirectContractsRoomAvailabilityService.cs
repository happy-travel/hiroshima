...
{
    public class DirectContractsRoomAvailabilityService: IDirectContractsRoomAvailabilityService
    {
        public DirectContractsRoomAvailabilityService(DirectContractsDbContext dbContext)
        {
            _dbContext = dbContext;
        }
        

        public async Task<List<RoomsGroupedByOccupationRequest>> GetGroupedRooms(List<AccommodationData> accommodations,
            AvailabilityRequest availabilityRequest)
        {
            var accommodationsDictionary = accommodations.ToDictionary(k => k.Accommodation.Id, v => v);

            var rooms = await _dbContext.GetAvailableRooms(
            ...
            //The temporary dictionary for grouping by room's occupation request
            ...
            //Check rooms' availability according an occupation request
            for (var i = 0; i < availabilityRequest.RoomDetails.Count; i++)
            {
                var roomOccupationRequest = availabilityRequest.RoomDetails[i];
                foreach (var room in rooms)
                {
                    if (IsRoomSuitableForRequest(roomOccupationRequest, room.OccupancyConfigurations,
            ...
            List<RoomsGroupedByOccupationRequest> GetResultFromDictionaryWithIds()
            {
                var roomsGroupedByOccupationRequests = new List<RoomsGroupedByOccupationRequest>();
                foreach (var idsDictionaryItem in roomsGroupedByAccommodationIdAndOccupancyId)
                {
                    ...
                    if (!DoesRoomsExistInAllOccupancyRequests(roomOccupationIdsWithRoomsDictionary))
                        continue;
                    ...
                    roomsGroupedByOccupationRequests.Add(new RoomsGroupedByOccupationRequest
                        {
                ...
                bool DoesRoomsExistInAllOccupancyRequests(Dictionary<int, List<Room>> roomOccupationIdsWithRoomsDictionary)
                {
                    for (var i = 0; i < availabilityRequest.RoomDetails.Count; i++)
                    {
